...
def makegrid(lat, lon, z_sta, stn, t, ssp_dir):
  ...
  ssp_stock_fname = './output/station_sound_speed_profiles/SSP_' + stn + '.txt'
  ...
  if not os.path.exists(ssp_stock_fname):
    ...
    if not os.path.exists(ssp_user_fname):
      ...
      ssp, z = get.lev_based_ssp(lat, lon, month + 1, levitus_dir, stn + '.txt')
  ...
  cor_fpath = './output/station_sound_speed_profiles/cor_rayline_' + stn +'.pkl'
