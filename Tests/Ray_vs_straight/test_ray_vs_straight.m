...
for ip = 1:Np
    ...
    %% compute rays
    try
        [rx,rz,Dr] = shootrays(p, v_profile, max(zmaxs), dr,vdz);
    end
...
for iz = 1:Nz
    plot(ax2,Dx_ray_z(:,iz)/nm2km,dlength_m(:,iz),'-o');
end
...
if ifsave
    save2jpg(11,'Test_ray_vs_straight_approx')
end
