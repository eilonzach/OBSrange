...
%     ylim(ax5,[1 max(misfit_r_xy)+10^(floor(log10(max(misfit_r_xy))))*10]);
    ylim(ax5,ax1.YLim);
    title(ax5,'\textbf{RMS}','interpreter','latex','fontsize',16);
    yticks(ax5,[0.001 0.01 0.1 1 10 100 1000]);
    ylabel(ax5,'$\mathbf{\delta r_{xy}\, (m)}$','fontsize',16,'Interpreter','latex')
...
%     ylim(ax6,[4 max(misfit_zsta)+10^(floor(log10(max(misfit_zsta))))*2]);
    ...
    ylabel(ax6,'$\mathbf{\delta Z\, (m)}$','fontsize',16,'Interpreter','latex')
...
%     ylim(ax7,[1 max(misfit_Vw)+10^(floor(log10(max(misfit_Vw))))]);
    ...
    ylabel(ax7,'$\mathbf{\delta V_{P} \, (m/s)}$','fontsize',16,'Interpreter','latex')
    ...
    plot(ax8,r_max(ifil),lam(ifil),symbol{ifil},'markerfacecolor',clr(ifil,:),'markersize',markersize); hold on;
    if ifil == 4
        plot(ax8,r_max(ifil),lam(ifil),symbol{ifil},'color','r','markersize',markersize,'linewidth',2); hold on;
    end
    if any(regexp(lgd{ifil},'line'))
    ...
    set(ax8,'yscale','linear','linewidth',1.5,'fontsize',16,'xminortick','on','yminortick','on','TickLength',[0.01, 0.001]*3,'layer','top');
...
%     yticks(ax8,[0.001 0.01 0.1 1 10 100 1e3 1e4 1e5 1e6 1e7]);
    ylabel(ax8,'{\boldmath$\lambda$ (\textbf{m}$\cdot$\textbf{Nm})}','fontsize',16,'Interpreter','latex')
    xlabel(ax8,'\bf{radius (Nm)}','fontsize',16,'Interpreter','latex')
    end
    %% Make shape legend
    dy = 0.958/(Nfils+0.25);
    ax(ifil) = axes('pos',[0.67 1-dy*ifil 0.05 0.05]);
...
dy_shift = 0.0;%-0.08;
dy = 1;%1.4;
ax5.Position = [ax5.Position(1)+dx_shift, ax5.Position(2)+dy_shift*0,   ax5.Position(3)*dx, ax5.Position(4)*dy];
ax6.Position = [ax6.Position(1)+dx_shift, ax6.Position(2)+dy_shift*1, ax6.Position(3)*dx, ax6.Position(4)*dy];
ax7.Position = [ax7.Position(1)+dx_shift, ax7.Position(2)+dy_shift*2, ax7.Position(3)*dx, ax7.Position(4)*dy];
...
ylabel(ax4,'$\mathbf{\nabla r_{xy}\, (m/Nm)}$','fontsize',16,'Interpreter','latex')
xlabel(ax4,'\bf{radius (Nm)}','fontsize',16,'Interpreter','latex')
