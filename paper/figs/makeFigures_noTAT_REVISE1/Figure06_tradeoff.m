...
%% Plot survey duration for each survey
f12 = figure(12); clf;
for ifil = 1:7
    h(ifil) = plot(dt(ifil)*60,misfit_r_xy(ifil),symbol{ifil},'markerfacecolor',clr(ifil,:),'markersize',markersize); hold on;
end
ylabel('$\mathbf{\delta r_{xy}\, (m)}$','fontsize',14,'Interpreter','latex')
xlabel('Survey duration (m)','fontsize',14,'Interpreter','latex')
set(gca,'yscale','log','linewidth',1.5,'fontsize',14,'xminortick','on','yminortick','on');
% ylim([0 65]);
