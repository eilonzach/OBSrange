...
\caption{ Model parameter misfits for 6 synthetic survey geometries of varying radii: \textit{PACMAN}, circle, cross, diamond, line, and triangle. Each survey geometry is shown to the left of its respective legend entry. The left column shows the mean absolute error (MAE) and right column shows the RMS misfit for 10,000 synthetic survey realizations for each survey type, where horizontal instrument location misfit is given by $\delta r_{xy} = \sqrt{\delta x_{O}^2 + \delta y_{O}^2} $. Open stars denote misfit in the direction running parallel to the line ($x$-direction for these tests). Red lines indicate the true RMS for each model parameter. We find that model parameters are most accurately recovered by the \textit{PACMAN} survey pattern with radius $\geq$1 Nm. }
