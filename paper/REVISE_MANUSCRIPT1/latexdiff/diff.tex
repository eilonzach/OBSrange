...
%DIF DEL OBSrange.tex      Sat Jan 19 10:24:02 2019
%DIF ADD ../OBSrange.tex   Sat Jan 19 10:23:10 2019
...
\DIFaddbegin \DIFadd{Here we outline the forward and inverse problems for inverting acoustic ranging data for instrument location following \mbox{%DIFAUXCMD
\citet{Creager1982}}%DIFAUXCMD
. }\DIFaddend We wish to locate an instrument which rests at unknown position and depth on the ocean floor (Figure~\ref{fig:cartoon}). Taking the drop coordinates as the center of a Cartesian coordinate system in which $x$ is positive towards East, $y$ is positive towards North, and $z$ is positive upwards from the sea surface, the instrument lies at location $(x,y,z)$. The time taken for an acoustic pulse to travel from the ship to the instrument and back is a function of the sound speed in water ($V_P$), and the location of the ship, as well as the ``turn-around time'' ($\tau$) that corresponds to the (fixed) processing time between the OBS transducer receiving a ping and sending its response. In detail, we can account for the possibility that if the ship is under way, its position changes between sending and receiving pings. Thus, the total travel time, $T$, is: 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{thebibliography}{12}
...
Accardo, N.~J., J.~B. Gaherty, D.~J. Shillington, C.~J. Ebinger, A.~A. Nyblade,
  ...
  \textit{209}, 1892--1905, \doi{10.1093/gji/ggx133}\DIFaddbegin \DIFadd{.
}
...
\DIFadd{Creager, K.~C., and L.~M. Dorman (1982), }{\DIFadd{Location of Instruments on the
  ...
  }\textit{\DIFadd{Journal of Geophysical Research}}\DIFadd{, }\textit{\DIFadd{87}}\DIFadd{, 8379--8388}\DIFaddend .
