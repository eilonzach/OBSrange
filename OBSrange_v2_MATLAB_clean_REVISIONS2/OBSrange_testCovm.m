...
save2pdf([modified_outdir,'/plots/',data.sta,'_4_bootstrapCovm.pdf'],f200,100)
end
...
x_std_bs(is) = std(x_sta)*2;
y_std_bs(is) = std(y_sta)*2;
z_std_bs(is) = std(z_sta)*2;
vw_std_bs(is) = std(V_w)*2;
covm_95 = sqrt(diag(Cm_mat(:,:,1)))*2;
x_std_covm(is) = covm_95(1);
y_std_covm(is) = covm_95(2);
z_std_covm(is) = covm_95(3);
vw_std_covm(is) = covm_95(4);
end
...
subplot(2,2,1);
plot(x_std_bs,x_std_covm,'or','linewidth',2); hold on;
plot([0 100],[0 100],'-k');
...
subplot(2,2,2);
plot(y_std_bs,y_std_covm,'or','linewidth',2); hold on;
plot([0 100],[0 100],'-k');
...
subplot(2,2,3);
plot(z_std_bs,z_std_covm,'or','linewidth',2); hold on;
plot([0 100],[0 100],'-k');
...
subplot(2,2,4);
plot(vw_std_bs,vw_std_covm,'or','linewidth',2); hold on;
plot([0 100],[0 100],'-k');
...
save2pdf('./Bootstrap_Covm_uncertainties.pdf',f300,100)
