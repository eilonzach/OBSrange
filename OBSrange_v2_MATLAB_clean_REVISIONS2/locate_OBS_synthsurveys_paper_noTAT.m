...
outdir = './OUT_OBSrange_synthsurveys_REVISION2_testAzimuthal/';
...
ifoverwrite = 1; % Overwrite results?

par = struct([]);
par(1).vp_w = 1500; % Assumed water velocity (m/s)
par.N_bs = 500; %1; %500; % Number of bootstrap iterations (= 1 for these tests)
...
% Correct for ray bending?
par.if_raycorrect = 0; % must be zero for synthetic tests
...
if exist([modified_outdir,'/mats/',surv,'_OUT.mat'],'file') == 2 && ~ifoverwrite
...
%     m0_strt(4,1) = par.TAT_start; %TAT;
    ...
    data(ii).x_sta = mean(x_sta);           if par.N_bs>1, data(ii).x_sta_std = std(x_sta); data(ii).x_sta1 = x_sta(1); end
    data(ii).y_sta = mean(y_sta);           if par.N_bs>1, data(ii).y_sta_std = std(y_sta); data(ii).y_sta1 = y_sta(1); end
    data(ii).z_sta = mean(z_sta);           if par.N_bs>1, data(ii).z_sta_std = std(z_sta); data(ii).z_sta1 = z_sta(1); end
    data(ii).TATsta = mean(TAT);            if par.N_bs>1, data(ii).TAT_std = std(TAT); end
    data(ii).V_w = mean(V_w);               if par.N_bs>1, data(ii).V_w_std = std(V_w); data(ii).V_w1 = V_w(1); end
    data(ii).drift = mean(drift);           if par.N_bs>1, data(ii).drift_std = std(drift); data(ii).drift1 = drift(1); end
