...
projpath = '/Users/russell/Lamont/PROJ_OBSrange/working/OBSrange/projects/PacificORCA_SynthBoot_surveys_strtmodel/'; % Josh PAPER
...
par = struct([]);
% Values to perturb starting model
par(1).dx_strt = 0;
par(1).dy_strt = 0;
par(1).dz_strt = -50;
dTAT_strt = 0.003;
...
par(1).vp_w = 1500 + dVp_strt; % Assumed water velocity (m/s)
...
par.TAT_start = 0.013 + dTAT_strt; % (s)
...
for ii = 1:length(data)
    ...
    m0_strt(1,1) = x_drop + par.dx_strt; %x0;
    m0_strt(2,1) = y_drop + par.dy_strt; %y0;
    m0_strt(3,1) = z_drop + par.dz_strt; %z0;
...
data(1).R_mat_mean = R_mat;
data(1).Cm_mat_mean = Cm_mat;
