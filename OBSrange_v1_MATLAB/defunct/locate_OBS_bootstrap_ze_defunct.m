...
%% Parameters
datafile = '../projects/PacificORCA/EE04.txt'; %CC10 'synth_obsloc', 'synth_obsloc_wbads'
outdir = '../projects/OUT_OBSrange/'; % output directory
is_saveout = 0; % save results and figures ?
par = struct([]);
...
npts_sm = 5; % number of points to include in moving average smoothing of ship velocity (1 = no smoothing);

% Ping QC -- Remove pings > ping_thresh ms away from neighbor
isQC_ping = 1; % Do quality control on pings?
...
v_ship = [moving_average(v_ship(1,:),npts_sm)'; moving_average(v_ship(2,:),npts_sm)'; moving_average(v_ship(3,:),npts_sm)'];

%% Set up initial model
m0_strt(1,1) = x_drop; %x0;
m0_strt(2,1) = y_drop; %y0;
m0_strt(3,1) = z_drop; %z0;
m0_strt(4,1) = TAT_start; %TAT;
m0_strt(5,1) = 0; %dvp;
...
% Set grid size
ngridpts = 40;
D = max([ std(x_sta) std(y_sta) std(z_sta) ]*6);
Dx = D;
Dy = D; 
Dz = D;
dx = 2*Dx/ngridpts;
...
for ix = 1:Nx
    for iy = 1:Ny
        for iz = 1:Nz
            % Apply scaling to par.vp_w and TAT to account for tradeoffs with Z
...
if is_saveout
    ...
    fprintf(fid,'\n===================================================\n');
    fprintf(fid,'%25s %15s %15s %15s %15s %15s \n','Lat','Lon','Range (m)','Residual (s)','Doplar Vel. (m/s)','TWT corr. (ms)');
    for ii = 1:Nobs
        fprintf(fid,'%10d: %15f %15f %15f %15f %15f %15f\n',ii,lats_ship(ii),lons_ship(ii),range(ii),dtwt_bs(ii)*1000,vr_bs(ii),dtwtcorr_bs(ii)*1000);
    end 
    fclose(fid);
